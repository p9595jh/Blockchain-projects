...
approveForMyOrg() {
  ...
  setGlobals $ORG_ADDR $ORG_NM $PEER_NM $ADMIN_NM $P0PORT
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.${SERV_NM}.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
checkCommitReadiness() {
  ...
  shift 1
  setGlobals $ORG
  infoln "Checking the commit readiness of the chaincode definition on ${PEER_NM}.${ORG_ADDR} on channel '$CHANNEL_NAME'..."
  local rc=1
  local COUNTER=1
  # continue to poll
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    sleep $DELAY
    infoln "Attempting to check the commit readiness of the chaincode definition on ${PEER_NM}.${ORG_ADDR}, Retry after $DELAY seconds."
    set -x
  ...
  if test $rc -eq 0; then
    infoln "Checking the commit readiness of the chaincode definition successful on ${PEER_NM}.${ORG_ADDR} on channel '$CHANNEL_NAME'"
  else
    fatalln "After $MAX_RETRY attempts, Check commit readiness result on ${PEER_NM}.${ORG_ADDR} is INVALID!"
  fi
}
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.${SERV_NM}.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.${SERV_NM}.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
infoln "Installing chaincode on peer0.cafe1..."
installChaincode cafe1 Cafe1 peer0 Admin 7051

queryInstalled cafe1 Cafe1 peer0 Admin 7051
approveForMyOrg cafe1 Cafe1 peer0 Admin 7051
commitChaincodeDefinition cafe1 Cafe1 peer0 Admin 7051
queryCommitted cafe1 Cafe1 peer0 Admin 7051

if [ "$CC_INIT_FCN" = "NA" ]; then
  infoln "Chaincode initialization is not required"
else
  chaincodeInvokeInit cafe1 Cafe1 peer0 Admin 7051
fi
