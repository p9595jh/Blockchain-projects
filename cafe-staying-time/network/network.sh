...
function createOrgs() {
  ...
  # infoln "testtttttttttttt"

  . organizations/fabric-ca/registerEnroll.sh

  while :
  do
    if [ ! -f "organizations/fabric-ca/cafe1/tls-cert.pem" ]; then
      sleep 1
    else
      break
    fi
  ...
  infoln "Create Cafe1 Identities"
  createOrg ${SERV_NM} cafe1 8054

  infoln "Create Orderer Org Identities"
  createOrderer ${SERV_NM}

  # generate connection profile
  infoln "Generate CCP files for organizations"
  ./organizations/ccp-generate.sh ${SERV_NM} cafe1 Cafe1 7051 8054 peer0
}
...
## call the script to join create the channel and join the peers of cafe1 and org2
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/cafe1/msp organizations/fabric-ca/cafe1/tls-cert.pem organizations/fabric-ca/cafe1/ca-cert.pem organizations/fabric-ca/cafe1/IssuerPublicKey organizations/fabric-ca/cafe1/IssuerRevocationPublicKey organizations/fabric-ca/cafe1/fabric-ca-server.db'
