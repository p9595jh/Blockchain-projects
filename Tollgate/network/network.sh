...
function createOrgs() {
  ...
  createOrdererOrg ${SERV_NM} 8054 orderer

  # generate connection profile
  infoln "Generate CCP files for organizations"
  ./organizations/ccp-generate.sh ${SERV_NM} tg1 Tg1 7051 7054 peer0
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/tg1/msp organizations/fabric-ca/tg1/tls-cert.pem organizations/fabric-ca/tg1/ca-cert.pem organizations/fabric-ca/tg1/IssuerPublicKey organizations/fabric-ca/tg1/IssuerRevocationPublicKey organizations/fabric-ca/tg1/fabric-ca-server.db'
